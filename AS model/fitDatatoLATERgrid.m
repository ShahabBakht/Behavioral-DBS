function [theta_hat,mu_hat,mu_stop_hat,sigma_hat,minval] = fitDatatoLATERgrid(LATENCYemp,param,numIter)%[theta_hat,mu_pro_hat,mu_anti_hat,mu_stop_hat,sigma_hat,minval] = fitDatatoLATERgrid(LATENCYemp,param,numIter)
...
function [theta_hatloc,mu_hatloc,mu_stop_hatloc,sigma_hatloc,minvalloc] = fitLocalDatatoLATERgrid(LATENCYemp,param)%[theta_hatloc,mu_pro_hatloc,mu_anti_hatloc,mu_stop_hatloc,sigma_hatloc,minvalloc] = fitLocalDatatoLATERgrid(LATENCYemp,param)
...
[I1,I2,I3,I4]=ind2sub(size(fitresult),minidx);
...
sigma_hatloc = param.sigma(I4);

end

function ks = LATENCYfitCostForGrid(LATENCYemp,theta,mu_stop,mu,sigma,numTrials)%ks = LATENCYfitCostForGrid(LATENCYemp,theta,mu_stop,mu_pro,mu_anti,sigma,numTrials)
param.theta = theta;
